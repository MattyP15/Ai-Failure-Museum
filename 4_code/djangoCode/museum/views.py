...
def submit_exhibit(request):
    ...
    if request.GET.get('test') == 'true':
        # We have to make a copy because request.POST is normally immutable
        from django.http import QueryDict
        mutable_post = QueryDict(mutable=True)
        mutable_post.update({
            'title': 'Hardcoded AI Failure',
            'description': 'This is a test submission generated by the view hack.',
            'category': '1', # Ensure this ID exists in your Category table!
        })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # If the form is invalid (e.g., category 1 doesn't exist), 
            # this will show you why in the UI.
